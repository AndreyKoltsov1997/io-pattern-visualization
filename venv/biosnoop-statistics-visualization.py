...
for yi, yy in enumerate(heatmap_y_values):
    ...
    if (process_name_fetch_result != "" or process_name_fetch_result != "PID"):
        ps_cmd_result = subprocess.run(['ps aux | grep {process_name}'.format(process_name=process_name_fetch_result)],
                                       shell=True,
    ...
    if (len(ps_cmd_raw_result) > 0):
        ...
        ps_cmd_result = subprocess.run(['ps aux | grep {process_name}'.format(process_name=process_name_fetch_result)], shell=True,
                                       stdout=subprocess.PIPE)
        ps_cmd_raw_result = ps_cmd_result.stdout.decode("utf-8").split()
        if (len(ps_cmd_raw_result) == 0):
            continue
